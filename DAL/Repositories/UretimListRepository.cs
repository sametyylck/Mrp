...
where mao.id=@id and moi.Tip='Malzemeler' and mao.DepoId=@DepoId
 ...
 moi.PlanlananMiktar,rez.RezerveDeger,SatinAlma.DurumBelirteci,SatinAlmaDetay.Miktar   
        ...
        public async Task<IEnumerable<ManufacturingOrderItemsIngredientsDetail>> IngredientsDetail(int CompanyId, int id)
        {
            ...
            left join Rezerve on Rezerve.StokId=Urunler.id  and Rezerve.Durum=1  and Rezerve.DepoId=@DepoId
			 left join Rezerve rez on rez.UretimId=mao.id and rez.UretimDetayId=moi.id  and rez.Durum=1  and rez.DepoId=@DepoId
            where mao.id=@id and moi.Tip='Malzemeler' and mao.DepoId=@DepoId  and mao.Durum!=3
        ...
        public async Task<IEnumerable<ManufacturingOrderDoneList>> ScheludeDoneList(ManufacturingOrderDoneListArama T, int CompanyId, int? KAYITSAYISI, int? SAYFA)
        {
			...
			Urunler.[Isim] AS UrunIsmi,Uretim.DepoId,DepoVeAdresler.Isim as DepoIsmi ,Uretim.BeklenenTarih,
            Kategoriler.[Isim] as  KategoriIsmi,ISNULL(Uretim.CariId,0) as CariId,ISNULL(Cari.AdSoyad,'') AS CariAdSoyad,
			Uretim.[Durum],
			...
			(UretimDetay.Tip = 'Operasyonlar' or  UretimDetay.Tip is null or   UretimDetay.Tip='Malzemeler')
            Group By Uretim.id, Uretim.[Isim],  Uretim.StokId,
			Uretim.CariId,Uretim.DepoId,Uretim.TamamlamaTarihi,
			Uretim.MalzemeFiyati,Cari.AdSoyad,Uretim.OperasyonFiyati,Uretim.ToplamMaliyet,
            DepoVeAdresler.Isim , Urunler.[Isim],Kategoriler.[Isim],Uretim.PlanlananMiktar,Uretim.[Durum],Uretim.BeklenenTarih) x
            where ISNULL(PlanlananZaman,0) like '%{T.PlanlananZaman}%' AND   ISNULL     (CariAdSoyad,'') like '%{T.CariAdSoyad}%' and
            ISNULL(UrunIsmi,'') like '%{T.UrunIsmi}%' and ISNULL(KategoriIsmi,'') like '%{T.KategoriIsmi}%' and ISNULL(PlanlananMiktar,'') like '%{T.PlanlananMiktar}%' and
			...
			ISNULL(Cari.AdSoyad,'') AS CariAdSoyad,Uretim.[Durum],
            ISNULL(Uretim.PlanlananMiktar,0)as PlanlananMiktar,
            SUM(ISNULL(UretimDetay.PlanlananZaman,0))as PlanlananZaman,
			ISNULL(Uretim.MalzemeFiyati,0)as MalzemeFiyati,ISNULL(Uretim.OperasyonFiyati,0) as OperasyonFiyati
			,ISNULL(Uretim.ToplamMaliyet,0)as ToplamMaliyet,Uretim.TamamlamaTarihi
			...
			(UretimDetay.Tip = 'Operasyonlar' or    UretimDetay.Tip is null or   UretimDetay.Tip='Malzemeler')
            Group By Uretim.id, Uretim.[Isim],  Uretim.StokId,
			Uretim.CariId,Cari.AdSoyad,Uretim.DepoId,Uretim.TamamlamaTarihi,
        ...
        public async Task<IEnumerable<ManufacturingOrderList>> ScheludeOpenList(ManufacturingOrderListArama T, int CompanyId, int? KAYITSAYISI, int? SAYFA)
        {
            ...
            if (T.BaslangıcTarih == null || T.SonTarih == null)
            {
                if (T.DepoId == null || T.DepoId == 0)
                {
                    sql = $@"DECLARE @KAYITSAYISI int DECLARE @SAYFA int SET @KAYITSAYISI ={KAYITSAYISI}  SET @SAYFA = {SAYFA}
            select x.* from (Select
            Uretim.id,Uretim.BeklenenTarih, Uretim.[Isim], Uretim.StokId, Urunler.[Isim] AS            UrunIsmi,Uretim.DepoId,DepoVeAdresler.Isim as DepoIsmi ,
            ISNULL(Kategoriler.[Isim],'') as  KategoriIsmi,ISNULL(Uretim.CariId,0) as CariId,ISNULL     (Cari.AdSoyad,'')    AS CariAdSoyad,Uretim.[Durum],
            ISNULL(Uretim.PlanlananMiktar,0)as PlanlananMiktar,
            SUM(ISNULL(UretimDetay.PlanlananZaman,0))as PlanlananZaman,
            ISNULL(Uretim.UretimTarihi,'') as UretimTarihi,
            ...
            where  Uretim.Aktif=1 and   Uretim.Durum!=3 and (UretimDetay.Tip = 'Operasyonlar' or    UretimDetay.Tip is null or   UretimDetay.Tip='Malzemeler')
            Group By Uretim.id, Uretim.[Isim], Uretim.StokId,Uretim.CariId,Cari.AdSoyad,Uretim.DepoId,
            DepoVeAdresler.Isim ,Uretim.BeklenenTarih,
            Urunler.[Isim],Kategoriler.            [Isim],Uretim.PlanlananMiktar,Uretim.UretimTarihi,Uretim.[Durum]) x
            where ISNULL(PlanlananZaman,0) like '%{T.PlanlananZaman}%' AND ISNULL(MalzemeDurum,0) like '%{T.MalzemeDurumu}%' and   ISNULL     (CariAdSoyad,'') like '%{T.CariAdSoyAd}%' and
            ISNULL(UrunIsmi,'') like '%{T.UrunIsmi}%' and ISNULL(KategoriIsmi,'') like '%{T.KategoriIsmi}%' and ISNULL(PlanlananMiktar,'') like '%{T.PlanlananMiktar}%' 
            ...
            ISNULL(Uretim.PlanlananMiktar,0)as PlanlananMiktar,
            SUM(ISNULL(UretimDetay.PlanlananZaman,0))as PlanlananZaman,
            ISNULL(Uretim.UretimTarihi,'') as ProductDeadline,
            ISNULL(min(UretimDetay.MalzemeDurum),0) as MalzemeDurum
            from Uretim
            left join UretimDetay on Uretim.id= UretimDetay.UretimId 
            left join DepoVeAdresler on DepoVeAdresler.id=Uretim.DepoId
            ...
            left join Urunler on Urunler.id=Uretim.StokId
            left join Kategoriler on Kategoriler.id=Urunler.KategoriId 
            where  Uretim.Aktif=1 and Uretim.DepoId=@location and Uretim.Durum!=3 and (UretimDetay.Tip = 'Operasyonlar' or    UretimDetay.Tip is null or   UretimDetay.Tip='Malzemeler')
            Group By Uretim.id, Uretim.[Isim],          Uretim.StokId,Uretim.CariId,Cari.AdSoyad,Uretim.DepoId,
            DepoVeAdresler.Isim ,Uretim.BeklenenTarih,
            Urunler.[Isim],Kategoriler.            [Isim],Uretim.PlanlananMiktar,Uretim.UretimTarihi,Uretim.[Durum]) x
            where ISNULL(PlanlananZaman,0) like '%{T.PlanlananZaman}%' AND ISNULL(MalzemeDurum,0) like '%{T.MalzemeDurumu}%' and  ISNULL     (CariAdSoyad,'') like '%{T.CariAdSoyAd}%' and
            ...
            else
            {
                ...
                if (T.DepoId == null || T.DepoId == 0)
                {
                    sql = $@"DECLARE @KAYITSAYISI int DECLARE @SAYFA int SET @KAYITSAYISI ={KAYITSAYISI}  SET @SAYFA = {SAYFA}
            select x.* from (Select
            Uretim.id,Uretim.BeklenenTarih, Uretim.[Isim], Uretim.StokId, Urunler.[Isim] AS            UrunIsmi,Uretim.DepoId,DepoVeAdresler.Isim as DepoIsim ,
            ISNULL(Kategoriler.[Isim],'') as  KategoriIsmi,ISNULL(Uretim.CariId,0) as CariId,ISNULL     (Cari.AdSoyad,'')    AS CariAdSoyad,Uretim.[Durum],
            ISNULL(Uretim.PlanlananMiktar,0)as PlanlananMiktar,
            SUM(ISNULL(UretimDetay.PlanlananZaman,0))as PlanlananZaman,
            ISNULL(Uretim.UretimTarihi,'') as ProductDeadline,
        ...
        public async Task<IEnumerable<ManufacturingTask>> TaskDoneList(ManufacturingTaskArama T, int CompanyId, int? KAYITSAYISI, int? SAYFA)
        {
            ...
            if (T.DepoId == null)
            {
                sql = $@"DECLARE @KAYITSAYISI int DECLARE @SAYFA int SET @KAYITSAYISI ={KAYITSAYISI}  SET @SAYFA = {SAYFA}
             select x.* from(
        select UretimDetay.id as id,Uretim.id as UretimId ,UretimDetay.KaynakId,Kaynaklar.[Isim]as KaynakIsmi,Uretim.[Isim]as OrderName,Uretim.UretimTarihi,Uretim.StokId,UretimDetay.TamamlamaTarihi, 
        Urunler.[Isim]as UrunIsmi,Uretim.PlanlananMiktar,UretimDetay.OperasyonId,Operasyonlar.[Isim]as OperasyonIsmi,UretimDetay.PlanlananZaman,UretimDetay.[Durum] from UretimDetay
        ...
        Group By Uretim.id,UretimDetay.id,UretimDetay.KaynakId,Kaynaklar.[Isim],Uretim.[Isim],Uretim.UretimTarihi,Uretim.StokId,UretimDetay.TamamlamaTarihi,
        ...
        left join Kaynaklar on Kaynaklar.id=UretimDetay.KaynakId
        left join Operasyonlar on Operasyonlar.id=UretimDetay.OperasyonId 
        where UretimDetay.CompanyId=@CompanyId and Uretim.id=UretimDetay.UretimId and UretimDetay.Durum=3  and Uretim.DepoId=@DepoId AND
        ISNULL(PlanlananZaman,0) like '%{T.PlanlananZaman}%'  and ISNULL(Kaynaklar.Isim,'') Like '%{T.KaynakIsmi}%' AND    ISNULL(Uretim.Isim,'') like '%{T.UretimIsmi}%' and
        ISNULL(Urunler.Isim,'') like '%{T.UrunIsmi}%' and ISNULL(Uretim.PlanlananMiktar,'') like '%{T.PlanlananMiktar}%' and ISNULL(Operasyonlar.Isim,'') like '%{T.OperasyonIsmi}%' 
        ...
        public async Task<IEnumerable<ManufacturingTask>> TaskOpenList(ManufacturingTaskArama T, int CompanyId, int? KAYITSAYISI, int? SAYFA)
        {
            ...
            if (T.DepoId == null)
            {
                sql = $@"DECLARE @KAYITSAYISI int DECLARE @SAYFA int SET @KAYITSAYISI ={KAYITSAYISI}  SET @SAYFA = {SAYFA}
             select x.* from(
        select UretimDetay.id as id,Uretim.id as UretimId,UretimDetay.KaynakId,Kaynaklar.[Isim]as KaynakIsmi,Uretim.[Isim]as OrderName,Uretim.UretimTarihi,Uretim.StokId,
        Urunler.[Isim]as UrunIsmi,Uretim.PlanlananMiktar,UretimDetay.OperasyonId,Operasyonlar.[Isim]as OperasyonIsmi,UretimDetay.PlanlananZaman,UretimDetay.[Durum] from UretimDetay
        left join Uretim on Uretim.id=UretimDetay.UretimId
        left join Urunler on Urunler.id=Uretim.StokId
        left join Kaynaklar on Kaynaklar.id=UretimDetay.KaynakId
        ...
        ISNULL(PlanlananZaman,0) like '%{T.PlanlananZaman}%'  and ISNULL(Kaynaklar.Isim,'') Like '%{T.KaynakIsmi}%' AND    ISNULL(Uretim.Isim,'') like '%{T.UretimIsmi}%' and
        ISNULL(Urunler.Isim,'') like '%{T.UrunIsmi}%' and ISNULL(Uretim.PlanlananMiktar,'') like '%{T.PlanlananMiktar}%' and ISNULL(Operasyonlar.Isim,'') like '%{T.OperasyonIsmi}%' 
        Group By Uretim.id,UretimDetay.KaynakId,Kaynaklar.[Isim],Uretim.[Isim],Uretim.UretimTarihi,Uretim.StokId,UretimDetay.id,
        Urunler.[Isim],Uretim.PlanlananMiktar,UretimDetay.OperasyonId,Operasyonlar.[Isim],UretimDetay.PlanlananZaman,UretimDetay.[Durum])x
        ORDER BY x.KaynakId OFFSET @KAYITSAYISI * (@SAYFA - 1) ROWS FETCH NEXT @KAYITSAYISI ROWS ONLY;";
            }
            else
        ...
        left join Kaynaklar on Kaynaklar.id=UretimDetay.KaynakId
        left join Operasyonlar on Operasyonlar.id=UretimDetay.OperasyonId 
        where Uretim.id=UretimDetay.UretimId and UretimDetay.Durum!=3  and Uretim.DepoId=@DepoId AND
        ISNULL(PlanlananZaman,0) like '%{T.PlanlananZaman}%'  and ISNULL(Kaynaklar.Isim,'') Like '%{T.KaynakIsmi}%' AND    ISNULL(Uretim.Isim,'') like '%{T.UretimIsmi}%' and
        ISNULL(Urunler.Isim,'') like '%{T.UrunIsmi}%' and ISNULL(Uretim.PlanlananMiktar,'') like '%{T.PlanlananMiktar}%' and ISNULL(Operasyonlar.Isim,'') like '%{T.OperasyonIsmi}%' 
        Group By Uretim.id,UretimDetay.KaynakId,Kaynaklar.[Isim],Uretim.[Isim],Uretim.UretimTarihi,Uretim.StokId,UretimDetay.id,
        Urunler.[Isim],Uretim.PlanlananMiktar,UretimDetay.OperasyonId,Operasyonlar.[Isim],UretimDetay.PlanlananZaman,UretimDetay.[Durum])x
